...
class TexObject( Logger ):
  ...
  *_appendix*
  ...
  class _TexObjectContextManager( object ):
  ...
  def __init__( self, d = {}, **kw ):
    ...
    self._keywords = { key :  val for key, val in d.iteritems() if not key.startswith('_') }
    self._keywords.update( { key :  val for key, val in self.__dict__.iteritems() if not key.startswith('_') } )
...
class TexObjectCollection( TexObject ):
  r"""
