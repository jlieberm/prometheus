...
def addTopLabels(can, legend
  ...
  moreText = []
  if binBounderies:
    moreText.append( getBinText(binBounderies, binStr, binUnit ) if not isinstance(binBounderies,basestring) else binBounderies )
...
class DrawerBase(Logger):
  ...
  def _etaBinsLegend(self):
    ...
    return map(getBinText, map(lambda x,y:(x,y),self._etaBins[:-1],self._etaBins[1:]), etaBinStr(l), etaBinUnit(l))
  ...
  def _etBinsLegend(self):
    ...
    return map(getBinText, map(lambda x,y:(x,y),bins[:-1],bins[1:]), etBinStr(l), etBinUnit(l))
