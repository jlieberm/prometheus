...
PathResolver::PR_find( const std::string& logical_file_name, const string& search_list,
  ...
  for (vector<string>::const_iterator itr = spv.begin();
   ...
   if( itr->find("http//")==0 && file_type==PR_regular_file && gSystem->Getenv("PATHRESOLVER_ALLOWHTTPDOWNLOAD") ) { //only http download files, not directories
      ...
      cout <<"Attempting http download of " << addr << "/" << file.string() << " to " << lp << endl;

      if(!is_directory(lpd)) {
         cout<<"   Creating directory: " << lpd  << endl;
         if(!boost::filesystem::create_directories(lpd)) {
            cout <<"Unable to create directories to write file to : " << lp << endl;
            continue;
      ...
      if(!TFile::Cp(fileToDownload.c_str(),(locationToDownloadTo+"/"+file.string()).c_str())) {
         cout<<"Unable to download file : " << fileToDownload << endl;
      } else {
         cout <<"Successfully downloaded " << fileToDownload << endl;
...
PathResolver::find_file(const std::string& logical_file_name,
  ...
  std::string path_list;
   ...
   if(envVarVal == nullptr) {
      cout <<search_path.c_str() << " environment variable not defined!" << endl;
...
{
  cout << "Trying to locate " << logical_file_name << endl;
  if(logical_file_name.compare(0, 4, "dev/")==0) {
      cout << "Locating dev file " << logical_file_name << ". Do not let this propagate to a release" << endl;
  }
  ...
  if(out=="") cout <<"Could not locate " << logical_file_name << endl;
   return out;
}
...
std::string PathResolver::find_calib_directory (const std::string& logical_file_name)
{
  cout <<"Trying to locate " << logical_file_name << endl;
  if(logical_file_name.compare(0, 4, "dev/")==0) {
     cout << "Locating dev directory " << logical_file_name << ". Do not let this propagate to a release" << endl;
  }
  //expand filename before finding 
  TString tmpString(logical_file_name);
  gSystem->ExpandPathName(tmpString);
