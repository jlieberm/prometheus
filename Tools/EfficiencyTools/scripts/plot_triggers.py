...
from EfficiencyTools.drawers import GetProfile, PlotProfiles
import argparse
import os
...
if args.reference:
  for paths in args.reference:
    files_ref+=expandFolders( paths ) 
for paths in args.test:
  files_test+=expandFolders( paths )
...
for trigItem in progressbar(triggerList, entries, step=step,logger=mainLogger, prefix='Making paths...'):
...
if args.reference:
  objects_ref = GetHistogramFromMany(files_ref, paths, keys, prefix='Getting reference...', debug=args.debug)
objects_test = GetHistogramFromMany(files_test, paths, keys, prefix='Getting test...', debug=args.debug)
...
### Plotting
for group in progressbar(triggerList_group, entries, step=step,logger=mainLogger):
